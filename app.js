...
function eventCard(e,now){
 ...
 return `<article class="event${e.category==='Kino'?' event-cinema':''}"><div class="date"><b>${ongoing?'DO':d.day}</b><span>${ongoing?dateParts(e.end).month:d.month}</span></div><div><span class="event-cat">${escapeHtml(e.category||'Akce')} · ${status}</span><h3>${escapeHtml(e.title)}</h3>${dateLabel?`<p class="event-range">${escapeHtml(dateLabel)}</p>`:''}<p class="event-meta">${escapeHtml(details)}</p><p>${escapeHtml(e.description||'Podrobnosti jsou na stránce pořadatele.')}</p>${e.price?`<p><b>Vstupné:</b> ${escapeHtml(e.price)}</p>`:''}<a href="${escapeHtml(e.source)}" target="_blank" rel="noopener">Podrobnosti a aktuální stav →</a></div></article>`;
}
