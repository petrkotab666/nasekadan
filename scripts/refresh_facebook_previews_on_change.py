...
def wait_until_exact_live(article_path: str, article: dict[str, str], timeout: int) -> dict[str, Any]:
    ...
    while time.time() < deadline:
        try:
            ...
            live.refresh_facebook_preview(article["url"])
            return {
                "og_image": live_image,
            ...
            last_error = str(exc)
            print(f"Čekám na přesný FB náhled {article_path}: {last_error}", file=sys.stderr)
            time.sleep(20)
...
def main() -> int:
    ...
    parser.add_argument(
        "--timeout",
        type=int,
        default=int(__import__("os").environ.get("FACEBOOK_WAIT_TIMEOUT", DEFAULT_TIMEOUT)),
    )
    ...
    if not candidates:
        write_status([{"state": "idle", "message": "Žádný publikovaný FB náhled nepotřebuje obnovu."}])
    ...
    for row in candidates:
        ...
        try:
            ...
            marker_data["preview_refreshed_at"] = datetime.now(timezone.utc).isoformat()
            ...
            print(f"Facebook náhled obnoven: {article_path} -> {live_result['og_image']}")
        except Exception as exc:
            failed = True
            error = queue.sanitize_error(exc)
            results.append(
            ...
            print(f"Obnova FB náhledu selhala pro {article_path}: {error}", file=sys.stderr)
