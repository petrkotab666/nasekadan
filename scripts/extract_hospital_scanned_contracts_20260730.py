...
def download(url: str, path: Path) -> None:
    ...
    with urllib.request.urlopen(req, timeout=180) as response, path.open("wb") as target:
...
def pdf_text_and_ocr(pdf: Path, stem: str) -> dict[str, object]:
    ...
    if used_ocr:
        ...
        subprocess.run(["pdftoppm", "-jpeg", "-r", "160", "-f", "1", "-l", str(min(pages or 30, 30)), str(pdf), str(prefix)], check=False)
        rendered = sorted(WORK.glob(f"{stem}-page-*.jpg"))
        parts: list[str] = []
        for index, image in enumerate(rendered, 1):
            outbase = WORK / f"{stem}-ocr-{index:03d}"
            subprocess.run(["tesseract", str(image), str(outbase), "-l", "ces+eng", "--psm", "6"], check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    # Přehledové listy stránek pro následnou vizuální kontrolu; bez originálních PDF.
    sheets: list[str] = []
    if not rendered and pages:
        prefix = WORK / f"{stem}-preview"
        subprocess.run(["pdftoppm", "-jpeg", "-r", "90", "-f", "1", "-l", str(min(pages, 20)), str(pdf), str(prefix)], check=False)
...
for rec_index, record in enumerate(selected, 1):
    ...
    report_lines.extend([f"## {title}", "", f"- Zveřejnění: {record.get('published') or 'neuvedeno'}", f"- Datum uzavření: {record.get('signed') or 'neuvedeno'}", f"- ID smlouvy/verze: {record.get('contract_id') or '?'} / {record.get('version_id') or '?'}"])
    attachment_results = []
    for att_index, url in enumerate(record.get("attachments", []), 1):
        ...
        local_pdf = WORK / filename
        try:
            download(str(url), local_pdf)
            sha256 = hashlib.sha256(local_pdf.read_bytes()).hexdigest()
            if local_pdf.suffix.lower() == ".pdf":
                details = pdf_text_and_ocr(local_pdf, stem)
            else:
                out_copy = OUT / f"{stem}{local_pdf.suffix.lower()}"
                shutil.copy2(local_pdf, out_copy)
