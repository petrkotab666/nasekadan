...
install -o root -g root -m 0755 /tmp/multisite_stats_v2.py "$APP_DIR/multisite_stats_v2.py"
install -o root -g root -m 0755 /tmp/patch_nginx.py "$APP_DIR/patch_nginx.py"
python3 -m py_compile "$APP_DIR/multisite_stats_v2.py" "$APP_DIR/patch_nginx.py"
...
location = /_nkstats/tracker.js {
  ...
  proxy_pass http://127.0.0.1:3226/tracker.js;
}
...
ExecStart=/usr/bin/python3 /usr/local/lib/multisite-stats/multisite_stats_v2.py
Restart=always
RestartSec=3
...
curl -fsS --max-time 3 http://127.0.0.1:3226/tracker.js | grep -q '_nkstats/hit.gif'
curl -fsS --max-time 3 'http://127.0.0.1:3226/hit.gif?site=multisite-selftest.invalid&path=/test&ref=' -o /tmp/multisite-hit.gif
test "$(wc -c < /tmp/multisite-hit.gif)" -gt 20
